<?php
/**
 * Heart Blockage Treatment District Pages Generator
 * Creates all 22 Haryana district pages for heart blockage treatment local SEO
 */
...
$mainDir = $basePath . '/heart-blockage-treatment';
...
foreach ($districts as $slug => $data) {
    ...
    $districtIndex = "<?php\n";
    $districtIndex .= "/**\n";
    $districtIndex .= " * Heart Blockage Treatment - {$data['display']} District Page\n";
    $districtIndex .= " * Target: Best Heart Blockage Treatment in {$data['display']}\n";
    $districtIndex .= " */\n\n";
    $districtIndex .= "\$district_name = '{$slug}';\n";
    $districtIndex .= "\$landmarks = ['" . implode("', '", $data['landmarks']) . "'];\n\n";
    $districtIndex .= "require_once '" . dirname(__DIR__) . "/components/heart-blockage-district-template.php';\n";
