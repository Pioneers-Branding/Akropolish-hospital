<?php
/**
 * Stroke Treatment District Pages Generator
 * Creates all 22 Haryana district pages for stroke treatment local SEO
 */
...
foreach ($districts as $slug => $data) {
    ...
    $districtIndex = "<?php\n";
    $districtIndex .= "/**\n";
    $districtIndex .= " * Stroke Treatment - {$data['display']} District Page\n";
    $districtIndex .= " * Target: Best Stroke Treatment in {$data['display']}\n";
    $districtIndex .= " */\n\n";
    $districtIndex .= "\$district_name = '{$slug}';\n";
    $districtIndex .= "\$landmarks = ['" . implode("', '", $data['landmarks']) . "'];\n\n";
    $districtIndex .= "require_once '" . str_repeat('..' . DIRECTORY_SEPARATOR, 3) . "components" . DIRECTORY_SEPARATOR . "stroke-district-template.php';\n";

    file_put_contents($districtDir . '/index.php', $districtIndex);
    echo "Created: stroke-treatment/haryana/{$slug}/index.php\n";
}

echo "\nAll 22 district pages created successfully!\n";
