  ...
  <div class="bg-white border-b shadow-sm">
    <div class="container mx-auto px-4 py-12">
      <div class="flex flex-col md:flex-row items-center gap-8">
        <img width="400" height="400" src="<?php echo $doctor['image']; ?>" class="w-48 h-48 rounded-full border-4 border-[#1E3A8A]/20 object-cover" />
        <div class="text-center md:text-left">
          <h1 class="text-4xl font-bold text-gray-900 mb-2">हरियाणा के सबसे प्रसिद्ध हड्डी के डॉक्टर</h1>
          <p class="text-[#1E3A8A] text-xl font-semibold mb-4">डॉ प्रतीक गिरोत्रा - प्रमुख, हड्डी रोग विभाग</p>
          <div class="flex flex-wrap justify-center md:justify-start gap-4">
            <a href="/contact" class="bg-[#1E3A8A] text-white px-6 py-3 rounded-xl font-bold flex items-center space-x-2">
              <?php echo getIcon('Calendar', 'h-5 w-5'); ?>
              <span>Appointment Book Karein</span>
            </a>
            <a href="tel:<?php echo CONTACT_INFO['emergency']; ?>" class="bg-[#EA580C] text-white px-6 py-3 rounded-xl font-bold">
    ...
    <div class="grid lg:grid-cols-3 gap-8">
        ...
        <div class="bg-white rounded-2xl shadow-lg p-8 border border-gray-100">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <?php foreach ($districts as $slug => $name): ?>
              <a href="/haddi-ka-doctor/haryana/<?php echo $slug; ?>" class="flex items-center space-x-3 p-3 rounded-lg border border-gray-100 hover:border-[#1E3A8A] hover:bg-blue-50 transition-all group">
                <div class="h-2 w-2 bg-[#1E3A8A] rounded-full"></div>
                <span class="text-gray-700 group-hover:text-[#1E3A8A] font-medium"><?php echo $name; ?> में सबसे अच्छे हड्डी के डॉक्टर</span>
              </a>
            <?php endforeach; ?>
          </div>
        </div>
      ...
      <div class="lg:col-span-1">
        <div class="bg-white rounded-2xl shadow-lg p-8 sticky top-24 border border-gray-100">
          ...
          <ul class="space-y-4">
            <li><a href="/doctors/dr-prateek-girotra" class="text-gray-600 hover:text-[#1E3A8A] flex items-center gap-2"><?php echo getIcon('User', 'h-4 w-4'); ?> Doctor Profile</a></li>
            <li><a href="/orthopedics" class="text-gray-600 hover:text-[#1E3A8A] flex items-center gap-2"><?php echo getIcon('Bone', 'h-4 w-4'); ?> Orthopedics Dept</a></li>
            <li><a href="/about" class="text-gray-600 hover:text-[#1E3A8A] flex items-center gap-2"><?php echo getIcon('Heart', 'h-4 w-4'); ?> About Hospital</a></li>
          </ul>
        </div>
      </div>
    </div>
